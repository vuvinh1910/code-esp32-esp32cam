...
// #include "plan_water_inferencing.h" // Thư viện AI của bạn
...
//     // Ví dụ: Camera chụp 240x240, model cần 96x96
...
//     // Không ép nhãn khi độ tin cậy quá thấp, tránh hiện tượng luôn ra cùng một lớp.
...
//     // 9. Giữ lại buffer để tái sử dụng cho lần suy luận sau, tránh phân mảnh heap.
// }
...
//     config.pixel_format = PIXFORMAT_JPEG; // Edge Impulse xử lý tốt nhất khi decode từ JPEG

//     // TỐI ƯU HÓA: Cài đặt khung hình nhỏ để tiết kiệm RAM cho AI
//     if(psramFound()){
//         config.frame_size = FRAMESIZE_QVGA; // 320x240 (Phù hợp để resize xuống 96x96)
//         config.jpeg_quality = 10;          
//         config.fb_count = 2;               
//     } else {
//         config.frame_size = FRAMESIZE_240X240; // Rất nhỏ gọn nếu không có PSRAM
...
//     server.begin();
//     Serial.println("Server đã chạy! Hãy gọi GET http://" + WiFi.localIP().toString() + "/api/recognize từ Backend.");
// }

// void loop() {
//     // Chỉ cần duy trì lắng nghe yêu cầu từ backend
//     server.handleClient();
//     delay(2);
// }